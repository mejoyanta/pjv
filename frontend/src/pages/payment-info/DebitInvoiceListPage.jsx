...
import { debitInvoiceService } from '../../services/paymentInfoService';
...
export default function DebitInvoiceListPage() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingItem) {
        await debitInvoiceService.update(editingItem.id, formData);
        setAlert({ type: 'success', message: 'Debit Invoice updated successfully!' });
      } else {
        await debitInvoiceService.create(formData);
        setAlert({ type: 'success', message: 'Debit Invoice created successfully!' });
      }
      setIsFormOpen(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await debitInvoiceService.delete(itemToDelete.id);
      setAlert({ type: 'success', message: 'Debit Invoice deleted successfully!' });
      setReloadTrigger(prev => prev + 1);
    } catch (err) {
      setAlert({ type: 'danger', message: err.message });
    } finally {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      await debitInvoiceService.downloadPdf();
    } catch (err) {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      render: (row) => (
        <span className="kt-badge kt-badge--inline kt-badge--warning" style={{ fontWeight: 600 }}>
          {row.challanNo || '—'}
        </span>
      )
    },
  ...
  return (
    <div>
      ...
      <DataTable
        title="Debit Invoice Records"
        columns={columns}
        fetchData={debitInvoiceService.load}
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: 600 }}>
            <div className="modal-header">
              <h5>{editingItem ? 'Edit Debit Invoice' : 'New Debit Invoice'}</h5>
              ...
              <div className="modal-body" style={{ padding: '20px' }}>
                ...
                <div className="form-group" style={{ marginBottom: 15 }}>
                  ...
                  <textarea
                    className="form-control"
                    rows="2"
                    placeholder="Debit invoice remarks..."
                    value={formData.note}
      ...
      <ConfirmModal
        isOpen={isDeleteOpen}
        title="Delete Debit Invoice"
        message={`Are you sure you want to delete debit invoice ${itemToDelete?.challanNo || itemToDelete?.id}? This action cannot be undone.`}
