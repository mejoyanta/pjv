...
import { priorityCustomerService } from '../../services/basicConfigService';
import { companyService } from '../../services/companyService';

export default function PriorityCustomerListPage() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingItem) {
        await priorityCustomerService.update(editingItem.id, formData);
        setAlert({ type: 'success', message: 'Priority Customer updated successfully!' });
      } else {
        await priorityCustomerService.create(formData);
        setAlert({ type: 'success', message: 'Priority Customer created successfully!' });
      }
      setIsFormOpen(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await priorityCustomerService.delete(itemToDelete.id);
      setAlert({ type: 'success', message: 'Priority Customer deleted successfully!' });
      setReloadTrigger(prev => prev + 1);
    } catch (err) {
      setAlert({ type: 'danger', message: err.message });
    } finally {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      await priorityCustomerService.downloadPdf();
    } catch (err) {
      ...
      render: (row) => (
        <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
          ...
          <button
            ...
            title="Delete Priority Customer"
            onClick={() => {
              setItemToDelete(row);
  ...
  return (
    <div>
        ...
        actions={
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="btn btn-brand" onClick={openCreateModal}>
              ...
              <span>Add Priority Customer</span>
            </button>
          </div>
        }
      />
      ...
      <DataTable
        title="Priority Customer Records"
        columns={columns}
        fetchData={priorityCustomerService.load}
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: 600 }}>
            <div className="modal-header">
              <h5>{editingItem ? 'Edit Priority Customer' : 'Add New Priority Customer'}</h5>
      ...
      {isViewOpen && viewItem && (
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: 500 }}>
            <div className="modal-header">
              <h5>Priority Customer Details</h5>
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete priority customer "${itemToDelete?.name}"?`}
