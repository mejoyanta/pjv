...
import { supplierService } from '../../services/basicConfigService';
...
export default function SupplierListPage() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingItem) {
        await supplierService.update(editingItem.id, formData);
        setAlert({ type: 'success', message: 'Supplier updated successfully!' });
      } else {
        await supplierService.create(formData);
        setAlert({ type: 'success', message: 'Supplier created successfully!' });
      }
      setIsFormOpen(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await supplierService.delete(itemToDelete.id);
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      await supplierService.downloadPdf();
    } catch (err) {
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        columns={columns}
        fetchData={supplierService.load}
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: 550 }}>
            <div className="modal-header">
              <h5>{editingItem ? 'Edit Supplier' : 'Add New Supplier'}</h5>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="modal-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                ...
                <button type="submit" className="btn btn-brand">{editingItem ? 'Update Supplier' : 'Create Supplier'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete supplier "${itemToDelete?.name}"?`}
