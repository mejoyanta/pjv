...
import { mushak610Service } from '../../services/mushakService';

export default function Musak610ListPage() {
  ...
  useEffect(() => {
    ...
    mushak610Service.load(params)
      ...
      .catch(err => {
        console.error('Failed to load Mushak 6.10:', err);
        setAlert({ type: 'danger', message: 'Failed to load Mushak 6.10 data.' });
      })
  ...
  const handlePdfDownload = async (id) => {
    try {
      ...
      await mushak610Service.downloadPdf(id);
    } catch (err) {
  ...
  return (
    <div className="kt-content kt-grid__item kt-grid__item--fluid" id="kt_content">
      <Subheader
        title="6.10 Mushak Forms"
        breadcrumbs={[
          { title: 'Mushak Form', url: '/mushak-form/musak-6-10' },
          { title: '6.10 Mushak Forms' }
        ]}
      />
      ...
      <div className="kt-portlet kt-portlet--mobile">
        <div className="kt-portlet__head kt-portlet__head--lg">
          <div className="kt-portlet__head-label">
            ...
            <h3 className="kt-portlet__head-title">Mushak 6.10 (Transactions Exceeding 200,000 BDT)</h3>
          </div>
              ...
              <div className="col-md-3">
                ...
                <div className="kt-input-icon kt-input-icon--left">
                  <input
                    type="text"
                    className="form-control"
                    placeholder="Search buyer, VI No..."
                    value={search}
                ...
                {loading ? (
                  <tr>
                    <td colSpan="14" className="text-center py-5 text-muted">
                      ...
                      <div className="mt-2">Loading Mushak 6.10 data...</div>
                    </td>
                  </tr>
                ) : data.length === 0 ? (
                  <tr>
                  ...
                  data.map((item, idx) => (
                    <tr key={item.id}>
                      ...
                      <td><code>{item.sellerBinNid || '—'}</code></td>
                      <td>{item.issueDate || '—'}</td>
                      <td>{item.buyerName || 'N/A'}</td>
                      <td>{item.buyerAddress || 'N/A'}</td>
                      <td><code>{item.buyerBinNid || 'N/A'}</code></td>
                      <td>{item.date || 'N/A'}</td>
                      <td><span className="badge badge-primary">{item.viNo}</span></td>
                      <td className="text-right font-weight-bold">{Number(item.sale || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title font-weight-bold">Mushak 6.10 Details - VI #{viewItem.viNo}</h5>
                <button type="button" className="close" onClick={() => setIsViewOpen(false)}>&times;</button>
              </div>
              <div className="modal-body">
                <div className="row mb-2">
                  <div className="col-md-6"><strong>Seller (Company):</strong> {viewItem.sellerName}</div>
                  <div className="col-md-6"><strong>Seller BIN/NID:</strong> {viewItem.sellerBinNid}</div>
                </div>
                <div className="row mb-2">
                  <div className="col-md-12"><strong>Seller Address:</strong> {viewItem.sellerAddress}</div>
                </div>
                <hr />
                <div className="row mb-2">
                  <div className="col-md-6"><strong>Buyer Name:</strong> {viewItem.buyerName || 'N/A'}</div>
                  <div className="col-md-6"><strong>Buyer BIN/NID:</strong> {viewItem.buyerBinNid || 'N/A'}</div>
                </div>
                <div className="row mb-2">
                  <div className="col-md-12"><strong>Buyer Address:</strong> {viewItem.buyerAddress || 'N/A'}</div>
                </div>
                <hr />
                <div className="row mb-2">
                  <div className="col-md-4"><strong>Supply Date:</strong> {viewItem.date}</div>
                  <div className="col-md-4"><strong>Challan / VI No:</strong> {viewItem.viNo}</div>
                  <div className="col-md-4"><strong>Total Amount:</strong> <span className="text-primary font-weight-bold">{Number(viewItem.sale || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>
                </div>
              </div>
