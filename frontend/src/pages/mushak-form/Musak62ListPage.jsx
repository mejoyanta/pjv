...
import { mushak62Service } from '../../services/mushakService';

export default function Musak62ListPage() {
  ...
  useEffect(() => {
    ...
    mushak62Service.load(params)
      ...
      .catch(err => {
        console.error('Failed to load Mushak 6.2:', err);
        setAlert({ type: 'danger', message: 'Failed to load Mushak 6.2 data.' });
      })
  ...
  const handlePdfDownload = async (id) => {
    try {
      ...
      await mushak62Service.downloadPdf(id);
    } catch (err) {
  ...
  return (
    <div className="kt-content kt-grid__item kt-grid__item--fluid" id="kt_content">
      <Subheader
        title="6.2 Mushak Forms"
        breadcrumbs={[
          { title: 'Mushak Form', url: '/mushak-form/musak-6-2' },
          { title: '6.2 Mushak Forms' }
        ]}
      />
      ...
      <div className="kt-portlet kt-portlet--mobile">
        <div className="kt-portlet__head kt-portlet__head--lg">
          <div className="kt-portlet__head-label">
            ...
            <h3 className="kt-portlet__head-title">Mushak 6.2 (Purchase Register)</h3>
          </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan="12" className="text-center py-5 text-muted">
                      ...
                      <div className="mt-2">Loading Mushak 6.2 data...</div>
                    </td>
                  </tr>
                ) : data.length === 0 ? (
                  <tr>
                  ...
                  data.map((item, idx) => (
                    <tr key={item.id}>
                      ...
                      <td>{item.date || 'N/A'}</td>
                      <td>{item.viNo ? <span className="badge badge-success">{item.viNo}</span> : 'N/A'}</td>
                      <td>{item.companyName || '—'}</td>
                      <td>{item.buyer || 'N/A'}</td>
                      <td>{item.productName || '—'}</td>
                      <td>{item.openingStock || '0.00'}</td>
                      <td>{item.closingStock != null ? item.closingStock : '0.00'}</td>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title font-weight-bold">Mushak 6.2 Details</h5>
              ...
              <div className="modal-body">
                <div className="row mb-2">
                  ...
                  <div className="col-md-6"><strong>Date:</strong> {viewItem.date}</div>
                </div>
                <div className="row mb-2">
                  <div className="col-md-6"><strong>VI No:</strong> {viewItem.viNo || 'N/A'}</div>
                  <div className="col-md-6"><strong>Buyer:</strong> {viewItem.buyer || 'N/A'}</div>
                </div>
                <div className="row mb-2">
                  <div className="col-md-6"><strong>Product:</strong> {viewItem.productName || '—'}</div>
                  <div className="col-md-6"><strong>Opening Stock:</strong> {viewItem.openingStock || '0.00'}</div>
                </div>
                <div className="row mb-2">
                  <div className="col-md-6"><strong>Closing Stock:</strong> {viewItem.closingStock || '0.00'}</div>
                  <div className="col-md-6"><strong>VAT Amount:</strong> {Number(viewItem.vatAmount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
                </div>
                <div className="row mb-2">
                  <div className="col-md-12"><strong>Total Sale Amount:</strong> <span className="text-primary font-weight-bold">{Number(viewItem.sale || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>
                </div>
              </div>
