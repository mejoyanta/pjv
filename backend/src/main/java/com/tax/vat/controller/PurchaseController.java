    ...
    public ApiResponse<Purchase> update(@PathVariable Long id, @RequestBody Purchase purchase) {
        return purchaseRepository.findById(id)
                .map(existing -> {
                    ...
                    if (purchase.getCd() != null) existing.setCd(purchase.getCd());
                    if (purchase.getRd() != null) existing.setRd(purchase.getRd());
                    ...
                    if (purchase.getVatType() != null) existing.setVatType(purchase.getVatType());
                    ...
                    if (purchase.getGrandTotal() != null) existing.setGrandTotal(purchase.getGrandTotal());
                    if (purchase.getBatchNo() != null) existing.setBatchNo(purchase.getBatchNo());
                    if (purchase.getVds() != null) existing.setVds(purchase.getVds());
                    if (purchase.getAddInto91() != null) existing.setAddInto91(purchase.getAddInto91());
                    if (purchase.getCountryRegion() != null) existing.setCountryRegion(purchase.getCountryRegion());
                    if (purchase.getAssessmentDate() != null) existing.setAssessmentDate(purchase.getAssessmentDate());
                    if (purchase.getBoeOfficeNo() != null) existing.setBoeOfficeNo(purchase.getBoeOfficeNo());
                    if (purchase.getCpcItemNo() != null) existing.setCpcItemNo(purchase.getCpcItemNo());
                    if (purchase.getBoeItem() != null) existing.setBoeItem(purchase.getBoeItem());
